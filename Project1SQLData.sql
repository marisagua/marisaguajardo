# Merging datasets to create dimension tables 

# creating df_actor table (to add film_id to actor table from film_actor table)
SELECT A.first_name, A.last_name, FA.actor_id, FA.film_id, FA.last_update
FROM sakila.actor AS A
LEFT OUTER JOIN sakila.film_actor AS FA
ON A.actor_id = FA.actor_id;

# creating df_category table (to add film_id to category table from film_category table)
SELECT c.category_id, c.name, c.last_update, fc.film_id
FROM sakila.category AS c
INNER JOIN sakila.film_category AS fc
ON c.category_id = fc.category_id;

# Saving the film table
